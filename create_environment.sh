...
# Create the directory structure if they do not exist (that is the purpose of the -p flag)
mkdir -p "$dir_name/app"
mkdir -p "$dir_name/modules"
mkdir -p "$dir_name/assets"
mkdir -p "$dir_name/config"

# Use a single script to create the directory structure and files
...
# Make scripts executable *AFTER* the files are  created
chmod u+x "$dir_name/startup.sh"
chmod u+x "$dir_name/app/reminder.sh"
chmod u+x "$dir_name/modules/functions.sh"

# Create the main directory last (only if it does not exist)
mkdir -p "$dir_name"
